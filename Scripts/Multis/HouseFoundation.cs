...
namespace Server.Multis
{
	...
	public class HouseFoundation : BaseHouse
	{
		...
		{
			int oldPrice = Price;
			int newPrice = oldPrice + 1000 + ((DesignState.Components.List.Length - CurrentState.Components.List.Length) * 50);
		...
		public static void Designer_Commit( NetState state, IEntity e, EncodedReader pvSrc )
		{
			...
			if ( context != null )
			{
				...
				int newPrice = oldPrice + 1000 + ((context.Foundation.DesignState.Components.List.Length - context.Foundation.CurrentState.Components.List.Length) * 50);
