...
namespace Server.Spells.Necromancy
{
	public class AnimateDeadSpell : NecromancerSpell
	{
        ...
        public static void ExternalTarget(Corpse c, Mobile Killer)
        {
            if (c == null)
            {
                    ...
                    if (group != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (map != null)
                            {
                                ...
                                Timer.DelayCall(TimeSpan.FromSeconds(2.0), new TimerStateCallback(SummonDelay_Callback), new object[] { Killer, c, p, map, group });
                            }
                            else
                                Killer.SendMessage("Null map error");
                        }
                    }
                }
            }
		...
		public static void Register( Mobile master, Mobile summoned )
		{
			...
			Timer.DelayCall( TimeSpan.FromSeconds( 5.0 ), TimeSpan.FromSeconds( 5.0 ), new TimerStateCallback( Summoned_Damage ), summoned );
		}

		private static void Summoned_Damage( object state )
		{
			Mobile mob = (Mobile)state;
            		int toDamage = mob.HitsMax / 100;
                    if (toDamage < 1)
                        toDamage = 1;
            		if (mob.Hits > toDamage)
                		mob.Hits -= toDamage;
            		else
                		mob.Kill();
		}
		...
		private static void SummonDelay_Callback( object state )
		{
            ...
            if (Spells.Third.BlessSpell.occido.Contains(caster))
                casterAbility += 200;
