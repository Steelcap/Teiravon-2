...
namespace Server.Spells.Second
{
	public class HarmSpell : Spell
	{
		...
		{
            if (Caster is TeiravonMobile &&( ((TeiravonMobile)Caster).IsCleric() || ((TeiravonMobile)Caster).IsPaladin() )&& ((TeiravonMobile)Caster).Faith == TeiravonMobile.Deity.Lloth)
        ...
        private class KinTimer : Timer
        {
            ...
            public KinTimer(Spell spell, Mobile from, Mobile targ, int duration, int damage)
            ...
            protected override void OnTick()
            {
                ...
                SpellHelper.Damage(m_Spell, m_Targ, Damage);
                //m_Targ.Damage(Damage,m_Cleric);

                RunCount -= 1;
                if (RunCount > 0)
                {
                    Timer m_Timer = new KinTimer(m_Spell,m_Cleric, m_Targ, RunCount, Damage);
                    m_Timer.Start();
                }
            }
        }
