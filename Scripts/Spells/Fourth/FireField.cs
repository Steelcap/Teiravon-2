		...
		private class InternalItem : Item
		{
			...
			public override bool OnMoveOver( Mobile m )
			{
				if ( Visible && m_Caster != null && (!Core.AOS || m != m_Caster) && SpellHelper.ValidIndirectTarget( m_Caster, m ) && m_Caster.CanBeHarmful( m, false ) )
				{
					...
					if ( m_Caster != null )
						damage = (int)( m_Caster.Skills[SkillName.Magery].Value / 20.0 + m_Caster.Skills[SkillName.EvalInt].Value / 20.0 + Utility.RandomMinMax( 1, 3) );
					else
					...
					else
					{
						...
						if ( map != null && caster != null )
						{
							...
							while ( m_Queue.Count > 0 )
							{
								...
								if ( caster != null )
									damage = (int)( caster.Skills[SkillName.Magery].Value / 20.0 + caster.Skills[SkillName.EvalInt].Value / 20.0 + Utility.RandomMinMax( 1, 3) );
								else
									damage = Utility.RandomMinMax( 1, 10 );
