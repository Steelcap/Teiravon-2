...
namespace Server
{
    public class AbilityCollection
    {
        ...
        public static void PrivateAreaEffect(TimeSpan delayMin, TimeSpan delayMax, double delayBreak, Map map, Point3D loc, int itemID, int duration, int hue, int renderMode, int range, int height, bool inToOut, bool ascending, bool descending, string tag)
        {
            for (int x = (range * -1); x <= range; ++x)
            {
                for (int y = (range * -1); y <= range; ++y)
                {
                    ...
                    if (dist <= range)
                    {
                        ...
                        if (inToOut)
                        {
                            ...
                            new PrivateEffectTimer(delayMin + TimeSpan.FromSeconds(dist * delayBreak), delayMax + TimeSpan.FromSeconds(dist * delayBreak), map, loc2, itemID, duration, hue, renderMode,tag).Start();
                        }
                        else
                        {
                            if (ascending)
                            ...
                            new PrivateEffectTimer(delayMin + TimeSpan.FromSeconds(range - (dist * delayBreak)), delayMax + TimeSpan.FromSeconds(range - (dist * delayBreak)), map, loc2, itemID, duration, hue, renderMode, tag).Start();
                        }
                    }
                }
            }
        ...
        public class PrivateEffectTimer : Timer
        {
            private int m_ItemID, m_Duration, m_Hue, m_RenderMode;
            private Map m_Map;
            private Point3D m_Location;
            ...
            public PrivateEffectTimer(TimeSpan delayMin, TimeSpan delayMax, Map map, Point3D loc, int itemID, int duration, int hue, int renderMode, string tag)
                : base(delayMin, delayMax)
            {
                m_ItemID = itemID;
                m_Duration = duration;
            ...
            public PrivateEffectTimer(TimeSpan delayMin, TimeSpan delayMax, Map map, Point3D loc, int itemID, int duration, string tag)
                : this(delayMin, delayMax, map, loc, itemID, duration, 0, 0, tag)
            {
            }

            protected override void OnTick()
            {
                SendPrivateLocationEffect(m_Location, m_Map, m_ItemID, m_Duration, 10, m_Hue, m_RenderMode, m_Tag);
                Stop();
            }
        }

        public static void SendPrivateLocationEffect(IPoint3D p, Map map, int itemID, int duration, int speed, int hue, int renderMode, string tag)
        {
             IEntity to = new Entity(Serial.Zero, new Point3D(p), map);

             IPooledEnumerable eable = map.GetMobilesInRange(to.Location);
            ...
            foreach (Mobile m in eable)
            {
                XmlData a = (XmlData)XmlAttach.FindAttachment(m, typeof(XmlData),tag);
                if (a != null)
                {
                    m.Send(new LocationEffect(p, itemID, speed, duration, hue, renderMode));
                }
            }
            eable.Free();
        }
