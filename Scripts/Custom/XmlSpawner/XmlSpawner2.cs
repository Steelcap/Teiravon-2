		...
		*/
		...
		public bool HasActiveSectors
		{
			get
			{
				...
				if (m_Region != null)
				{
					ArrayList players = m_Region.Players;
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
		...
		public static void Initialize()
		{
			...
			Server.Commands.Register("XmlSpawnerLoad", DiskAccessLevel, new CommandEventHandler(Load_OnCommand));
			Server.Commands.Register("XmlSpawnerSave", DiskAccessLevel, new CommandEventHandler(Save_OnCommand));
			Server.Commands.Register("XmlSpawnerSaveAll", DiskAccessLevel, new CommandEventHandler(SaveAll_OnCommand));
			//added respawn commands
			Server.Commands.Register("XmlSpawnerRespawn", AccessLevel.GameMaster, new CommandEventHandler(Respawn_OnCommand));
			Server.Commands.Register("XmlSpawnerRespawnAll", AccessLevel.GameMaster, new CommandEventHandler(RespawnAll_OnCommand));

			// ok, I'm lazy. I dont like all that typing, so these are two aliases for the longer commands
			Server.Commands.Register("XmlShow", AccessLevel.Administrator, new CommandEventHandler(ShowSpawnPoints_OnCommand));
			Server.Commands.Register("XmlHide", AccessLevel.Administrator, new CommandEventHandler(HideSpawnPoints_OnCommand));

			Server.Commands.Register("XmlHome", AccessLevel.GameMaster, new CommandEventHandler(XmlHome_OnCommand));
			Server.Commands.Register("XmlUnLoad", DiskAccessLevel, new CommandEventHandler(UnLoad_OnCommand));
			Server.Commands.Register("XmlSpawnerUnLoad", DiskAccessLevel, new CommandEventHandler(UnLoad_OnCommand));
			Server.Commands.Register("XmlLoad", DiskAccessLevel, new CommandEventHandler(Load_OnCommand));
			Server.Commands.Register("XmlLoadHere", DiskAccessLevel, new CommandEventHandler(LoadHere_OnCommand));
			Server.Commands.Register("XmlNewLoad", DiskAccessLevel, new CommandEventHandler(NewLoad_OnCommand));
			Server.Commands.Register("XmlNewLoadHere", DiskAccessLevel, new CommandEventHandler(NewLoadHere_OnCommand));
			Server.Commands.Register("XmlSave", DiskAccessLevel, new CommandEventHandler(Save_OnCommand));
			Server.Commands.Register("XmlSaveAll", DiskAccessLevel, new CommandEventHandler(SaveAll_OnCommand));
			Server.Commands.Register("XmlSaveOld", DiskAccessLevel, new CommandEventHandler(SaveOld_OnCommand));
			Server.Commands.Register("XmlImportSpawners", DiskAccessLevel, new CommandEventHandler(XmlImportSpawners_OnCommand));
			Server.Commands.Register("XmlImportMSF", DiskAccessLevel, new CommandEventHandler(XmlImportMSF_OnCommand));
			Server.Commands.Register("XmlImportMap", DiskAccessLevel, new CommandEventHandler(XmlImportMap_OnCommand));
			Server.Commands.Register("XmlDefaults", AccessLevel.Administrator, new CommandEventHandler(XmlDefaults_OnCommand));
			Server.Commands.Register("XmlGet", AccessLevel.GameMaster, new CommandEventHandler(XmlGetValue_OnCommand));
			Server.Scripts.Commands.TargetCommands.Register(new XmlSetCommand());
			//Server.Commands.Register( "XmlSet", AccessLevel.GameMaster, new CommandEventHandler( XmlSetValue_OnCommand ) );
			Server.Commands.Register("OptimalSmartSpawning", AccessLevel.Administrator, new CommandEventHandler(OptimalSmartSpawning_OnCommand));
			Server.Commands.Register("SmartStat", AccessLevel.GameMaster, new CommandEventHandler(SmartStat_OnCommand));
			Server.Commands.Register("XmlGo", AccessLevel.GameMaster, new CommandEventHandler(SpawnEditorGo_OnCommand));
			//Server.Commands.Register( "TagList", AccessLevel.Administrator, new CommandEventHandler( ShowTagList_OnCommand ) );

#if(TRACE)
			Server.Commands.Register( "XmlMake", AccessLevel.Administrator, new CommandEventHandler( XmlMake_OnCommand ) );
			Server.Commands.Register( "XmlTrace", AccessLevel.Administrator, new CommandEventHandler( XmlTrace_OnCommand ) );
			Server.Commands.Register( "XmlResetTrace", AccessLevel.Administrator, new CommandEventHandler( XmlResetTrace_OnCommand ) );
#endif
		...
		private class XmlHomeTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				if (targeted is XmlSpawner)
				{
					...
					{
						BaseCreature m = targeted as BaseCreature;
						foreach (Item item in World.Items.Values)
						{
							if (item is XmlSpawner)
							{
								XmlSpawner xml_item = item as XmlSpawner;
								// could be the one
								// so look through the spawn list to see if the mob is there
								foreach (SpawnObject so in xml_item.m_SpawnObjects)
								{
									for (int x = 0; x < so.SpawnedObjects.Count; x++)
									{
										object o = so.SpawnedObjects[x];
										...
										if (o == targeted)
										{
											from.SendMessage("{0}, {1}, {2}", xml_item.X, xml_item.Y, xml_item.Z);

											if (m_e.GetString(0) == "go")
											{
												// make sure the spawner is not in a container.
												if (xml_item.Parent == null)
												{
													from.Location = new Point3D(xml_item.Location);
													from.Map = xml_item.Map;
												}
												else
												{
												...
												if (m_e.GetString(0) == "send")
												{
													...
													if (xml_item.Parent == null)
													{
														m.Location = new Point3D(xml_item.Location);
														m.Map = xml_item.Map;
													}
													else
													{
						...
						if (targeted is Item)
						{
							...
							foreach (Item item in World.Items.Values)
							{
								if (item is XmlSpawner)
								{
									...
									int mx = m.Location.X;
									int my = m.Location.Y;
									int ix = xml_item.Location.X;
									int iy = xml_item.Location.Y;
									int xrange = xml_item.m_Width + 1;
									int yrange = xml_item.m_Height + 1;

									// check to see whether this spawner is within homerange + spawnrange of the mob which should
									// be the max distance the mob can be from it
									if ((mx > ix - xrange) && (mx < ix + xrange) && (my > iy - yrange) && (my < iy + yrange))
									{
										// could be the one
										// so look through the spawn list to see if the mob is there
										foreach (SpawnObject so in xml_item.m_SpawnObjects)
												...
												if (o == targeted)
												{
													...
													if (m_e.GetString(0) == "go")
													{
														...
														if (m_e.GetString(0) == "send")
														{
															...
															if (xml_item.Parent == null)
															{
																m.MoveToWorld(new Point3D(xml_item.Location), xml_item.Map);
															}
															else
															{
																from.SendMessage("Spawner is in a container");
				...
				for (int x = 0; x < so.SpawnedObjects.Count; x++)
				{
					...
					else if (o is Mobile)
					{
						...
						else if (m is BaseCreature)
						{
							...
							if (b.Controled || b.IsStabled || (b.Owners != null && b.Owners.Count > 0))
							{
								so.SpawnedObjects.Remove(o);
								x--;
								removed = true;
		...
		public bool Spawn(int index, bool smartspawn, int packrange, Point3D packcoord, bool ignoreloopprotection)
		{
			...
			if (TheSpawn != null)
			{
				...
				ArrayList spawnpositioning = null;
				...
				while (substitutedtypeName.StartsWith("#"))
				{
					...
					if (args.Length > 0)
					{
						...
						{
							spawnpositioning = new ArrayList();
						}
		...
		private void FindTileLocations(ref ArrayList locations, Map map, int startx, int starty, int width, int height, ArrayList includetilelist, ArrayList excludetilelist, TileFlag tileflag)
		{
		...
		private void FindRegionTileLocations(ref ArrayList locations, Region r, ArrayList includetilelist, ArrayList excludetilelist, TileFlag tileflag)
		{
			if (r == null || r.Coords == null) return;

			int count = r.Coords.Count;
		...
		// used for getting non-mobile spawn positions
		public Point3D GetSpawnPosition(bool requiresurface, int packrange, Point3D packcoord, ArrayList spawnpositioning)
		{
			return GetSpawnPosition(requiresurface, packrange, packcoord, spawnpositioning, null);
		}

		public Point3D GetSpawnPosition(bool requiresurface, int packrange, Point3D packcoord, ArrayList spawnpositioning, Mobile mob)
		{
			Map map = Map;
			...
			ArrayList includetilelist = null;
			ArrayList excludetilelist = null;
				...
				if (packrange >= 0 && packcoord != Point3D.Zero)
				{
					...
					if (m_Region != null && HasRegionPoints(m_Region))	// 2004.02.08 :: Omega Red
					{
						...
						if (includetilelist != null || excludetilelist != null || tileflag != TileFlag.None)
						{
							...
							{
								Point3D p = (Point3D)locations[Utility.Random(locations.Count)];
								x = p.X;
								y = p.Y;
								defaultZ = p.Z;
							}
