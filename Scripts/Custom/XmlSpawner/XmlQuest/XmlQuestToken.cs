...
namespace Server.Items
{
	...
	public class XmlQuestTokenPack : Container
	{
		...
		public XmlQuestTokenPack() : base( 0x9B2 )
		{
			Weight = 0;
		}
	...
	public abstract class XmlQuestToken : Item, IXmlQuest
	{
		...
		public XmlQuestToken( Serial serial ) : base( serial )
		{
		}
		...
		private void CalculateWeight(Item target)
		{
			if(target is Container)
			{
				...
				((Container)target).TotalWeight = weight;
				((Container)target).TotalGold = gold;
				((Container)target).TotalItems = nitems;
			}
		}
		...
		private void UpdateWeight()
		{
			...
			if(m_RewardItem != null && !m_RewardItem.Deleted && PlayerMade)
			{
				...
				{
					TotalWeight = m_RewardItem.TotalWeight + (int)m_RewardItem.Weight;
					TotalGold = m_RewardItem.TotalGold;

				} 
				else
				{
					TotalWeight = (int)(m_RewardItem.Weight * m_RewardItem.Amount);
					TotalGold = m_RewardItem.TotalGold;
