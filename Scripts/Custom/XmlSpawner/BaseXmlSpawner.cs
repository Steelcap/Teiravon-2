...
namespace Server.Mobiles
{
    ...
    public class BaseXmlSpawner
    {
                    ...
                    if (range > 0)
                    {
                        ...
                        if (rangelist != null)
                        {
                            foreach (Mobile p in rangelist)
                            {
                                ...
                                else if (petres && p is BaseCreature && ((BaseCreature)p).ControlMaster == triggermob && ((BaseCreature)p).Controled && ((BaseCreature)p).IsDeadPet)
                                {
        ...
        public static void SendBoltEffect(IEntity e, int sound, int hue)
        {
            ...
            foreach (NetState state in eable)
            {
                ...
                {
                    if (playSound == null)
                        playSound = new PlaySound(sound, e);
    ...
    string propertyString, out string status_str)
        ...
        public static void AddSpawnItem(XmlSpawner spawner, XmlSpawner.SpawnObject theSpawn, Item item, Point3D location, Map map, Mobile trigmob, bool requiresurface,
            ArrayList spawnpositioning, string propertyString, out string status_str)
        {
        ...
        public static void AddSpawnItem(XmlSpawner spawner, XmlSpawner.SpawnObject theSpawn, Item item, Point3D location, Map map, Mobile trigmob, bool requiresurface,
            ArrayList spawnpositioning, string propertyString, bool smartspawn, out string status_str)
        {
        ...
        */
        ...
        public static bool SpawnTypeKeyword(object invoker, XmlSpawner.SpawnObject TheSpawn, string typeName, string substitutedtypeName, bool requiresurface,
            ArrayList spawnpositioning, Mobile triggermob, Point3D location, Map map, out string status_str)
        {
        ...
        public static bool SpawnTypeKeyword(object invoker, XmlSpawner.SpawnObject TheSpawn, string typeName, string substitutedtypeName, bool requiresurface,
            ArrayList spawnpositioning, Mobile triggermob, Point3D location, Map map, XmlGumpCallback gumpcallback, out string status_str)
        {
            status_str = null;
                    ...
                    case typeKeyword.SETONPETS:
                        {
                            ...
                            foreach (object nearbyobj in nearbylist)
                            {
                                ...
                                if (pet != null && pet.Controled && pet.ControlMaster == triggermob)
                                {
                                    ApplyObjectStringProperties(spawner, substitutedtypeName, nearbyobj, triggermob, invoker, out status_str);
                                }
                            }
                    ...
                    case typeKeyword.COMMAND:
                        {
                            ...
                            if (arglist.Length > 0)
                            {
                                ...
                                if (CommandMobileName != null)
                                {
                                    ...
                                    if (dummy != null)
                                    {
                                        Commands.Handle(dummy, String.Format("{0}{1}", Commands.CommandPrefix, arglist[1]));
                                    }
                                }
                                else
                                    if (triggermob != null && !triggermob.Deleted)
                                    {
                                        Commands.Handle(triggermob, String.Format("{0}{1}", Commands.CommandPrefix, arglist[1]));
                                    }
