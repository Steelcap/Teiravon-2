...
namespace Server.Engines.PlayerDonation
{
	...
	public class DonationStore
	{
		...
		private static string
			...
			DatabaseServer = "localhost",	// your MySQL database hostname
			DatabaseName = "donation_db",			// the database name of your donation store db
			DatabaseUserID = "root",				// username for your MySQL database access
			DatabasePassword = "teirSQL";	// password
        ...
        {
            Commands.Register("donations", AccessLevel.Player, new CommandEventHandler(ClaimAllDonationItems_OnCommand));
        }

        public static void ClaimAllDonationItems_OnCommand(CommandEventArgs e)
        {
            e.Mobile.SendGump(new DonationStoreGump(e.Mobile));
        }
