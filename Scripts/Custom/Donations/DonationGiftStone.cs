...
namespace Server.Gumps
{
	public class DonationStoreGump : Gump
	{
		...
		{
			string username = ((Account)acct.Account).Username;
		...
		public override void OnResponse( Server.Network.NetState sender, RelayInfo info )
		{
			if (info.ButtonID > 0 && info.ButtonID <= 5)
			{
				...
				IEntity gift = DonationStore.RedeemGift(giftId, ((Account)m_From.Account).Username);
				if (gift != null)
				{
					m_From.AddToBackpack((Item)gift);
					m_From.SendMessage("You've recieved {0}. Thank you for your donation!", ((Item)gift).Name);
				}
			}
		}
