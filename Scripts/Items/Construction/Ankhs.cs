...
namespace Server.Items
{
	public class Ankhs
	{
		...
		public static void Resurrect( Mobile m, Item item )
		{
			...
			TimeSpan ts = m_Player.NextEXPLoss - DateTime.Now;

			if ( !m.InRange( item.GetWorldLocation(), ResurrectRange ) )
				m.SendLocalizedMessage( 500446 ); // That is too far away.

			else if ( ts > TimeSpan.Zero && m.Map != null && m.Map.CanFit( m.Location, 16, false, false ) )
			{
				m_Player.Resurrect();

				if ( m_Player.AccessLevel > AccessLevel.Player )
					m_Player.SendMessage( "DEBUG: Next EXP Loss at {0}", ts.ToString() );
			}
			else if ( m.Map != null && m.Map.CanFit( m.Location, 16, false, false ) )
			{
				...
				if ( !m_Player.Young )
				{
					temp = m_Player.PlayerExp - (float)( (m_Player.PlayerExp * ( 21 - m_Player.PlayerLevel ) * 0.01) / 8 );

					if ( temp < 0 )
					temp = 0;

					m_Player.PlayerExp = (int)temp;

					m_Player.SendMessage( "You have lost some experience." );

					temp2 = m_Player.Fame - (float)( (m_Player.Fame * ( 46 - m_Player.PlayerLevel ) * 0.01) / 4 );

					if ( temp2 < 0 )
						temp2 = 0;

					m_Player.Fame = (int)temp2;

					m_Player.SendMessage( "You have lost fame." );

					m_Player.Resurrect();

					try{ m_Player.NextEXPLoss = DateTime.Now + TimeSpan.FromMinutes( 30.0 ); }
					catch{}
				...
				{
					m_Player.Resurrect();
					m_Player.SendMessage( "You have been resurrected. Thanks to you being new to this realm you have not lost any experience points." );
					m_Player.SendMessage( "After your young status expires, you will loose experience points or you may even not be able to resurrect again." );
					m_Player.SendMessage( "Caution is strongly advised." );
				}
