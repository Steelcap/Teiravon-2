...
namespace Server.Items
{
	...
	public abstract class BaseAddon : Item, IChopable, IAddon
	{
		...
		public virtual void OnChop( Mobile from )
		{
			...
			else if (  staticHouse != null && staticHouse.IsOwner( from ) )//&& staticHouse.Addons.Contains( this ) )
			{
		...
		public AddonFitResult CouldFit( IPoint3D p, Map map, Mobile from, ref ArrayList houseList )
		{
			...
			foreach ( AddonComponent c in m_Components )
			{
				...
				if (!(this is OrcCombatBridgeAddon))
				{
		...
		public bool CheckHouse( Mobile from, Point3D p, Map map, int height, ArrayList list )
		{
			...
			if (this is BallistaEastAddon || this is OrcPalisadeSouthAddon || this is OrcPalisadeEastAddon || this is DwarvenForgeAddon)
				return true;

			if ( from.Region is StaticHouseRegion )
			{
				StaticHouseSign sHouse = StaticHouseSign.StaticFindHouseAt( p, map, height, from );
				if (!sHouse.IsOwner(from))
					return false;
				return true;
			}
