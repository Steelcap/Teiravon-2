	...
	public abstract class BaseBeverage : Item, IHasQuantity
	{
		...
		public virtual void Pour_OnTarget( Mobile from, object targ )
		{
			...
			if ( targ is BaseBeverage )
			{
				...
				//STARMOD: Beverage edit
					from.Emote( "*pours some {1} into a {0}*", ((bev.GetType()).Name).ToLower(), ((bev.Content).ToString()).ToLower() );
				//ENDMOD: Beverage edit
			...
			else if ( from == targ )
			{
				...
				//STARMOD: Beverage edit
				from.Emote( "*drinks some {0}*", ((this.Content).ToString()).ToLower() );
