...
namespace Server.Items
{
	public abstract class BaseArmor : Item, IScissorable, IFactionItem, ICraftable
	{
		...
		public virtual double ArmorRating
		{
			get
			{
				...
				switch ( m_Resource )
				{
					...
					case CraftResource.Mithril:			ar += 18; break;
					case CraftResource.Bloodrock:		ar += 18; break;
					case CraftResource.Steel:			ar += 18; break;
					case CraftResource.Adamantite:		ar += 18; break;
					case CraftResource.Ithilmar:		ar += 18; break;
					case CraftResource.SpinedLeather:	ar += 10; break;
                    case CraftResource.TuftedLeather:   ar += 12; break;
					case CraftResource.HornedLeather:	ar += 13; break;
					case CraftResource.BarbedLeather:	ar += 16; break;
                    case CraftResource.ScaledLeather:   ar += 18; break;
				}
		...
		public AosArmorAttributes ArmorAttributes
		{
			...
			set{m_AosArmorAttributes = value;}
		}
...
//			CraftSystem system = DefTailoring.CraftSystem;
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					if ( m_Resource == CraftResource.None )
					{
                        ...
                        else if (mat == ArmorMaterialType.Tufted)
                            m_Resource = CraftResource.TuftedLeather;
		...
		public override bool CanEquip( Mobile from )
		{
			...
			if ( !m_Player.HasFeat( TeiravonMobile.Feats.ArmorProficiency ) && MaterialType == ArmorMaterialType.Plate && m_Player.AccessLevel < AccessLevel.GameMaster)
			{
				m_Player.SendMessage( "You must have the Armor Proficiency feat to wear this." );
		...
		public virtual int OnHit( BaseWeapon weapon, int damageTaken )
		{
			...
			if ( 35 > Utility.Random( 100 ) ) // 35% chance to lower durability
			{
				int RepairHitPoints = 0;
				
				if ( Core.AOS && m_AosArmorAttributes.SelfRepair > Utility.Random( 10 ) )
				{
					RepairHitPoints = (int)(m_AosArmorAttributes.SelfRepair / 2) + 1;
					HitPoints += RepairHitPoints;
				...
				else
				{
					...
					if ( weapon.Type == WeaponType.Bashing )
                        wear = Utility.Random( 3 );
					else
    					wear = Utility.Random( 2 );

                    wear = (int)(wear * Math.Log(damageTaken));
		...
		public override void AddNameProperty( ObjectPropertyList list )
		{
			...
			else
			{
				switch ( m_Resource )
				{
                    ...
                    case CraftResource.TuftedLeather:   oreType = "tufted leather"; break; // tufted
                    case CraftResource.ScaledLeather:   oreType = "scaled leather"; break; // scaled
...
//					list.Add( Name );
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			//TEIRAVON: Armor classification
			if ( MaterialType == ArmorMaterialType.Plate )
				list.Add( 1060658, "{0}\t{1}", "Protection", "Heavy" );
            else if (MaterialType == ArmorMaterialType.Chainmail || MaterialType == ArmorMaterialType.Dwarven || MaterialType == ArmorMaterialType.Ringmail)
				list.Add( 1060658, "{0}\t{1}", "Protection", "Medium" );
			else if ( MaterialType != ArmorMaterialType.Cloth && !(this is BaseShield))
				list.Add( 1060658, "{0}\t{1}", "Protection", "Light" );
			//
		...
		public int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue )
		{
			...
			int xtraskillbonus = (int)(((int)from.Skills.Blacksmith.Base - 100)/4);
			if (xtraskillbonus < 0)
				xtraskillbonus = 0;
			DistributeBonuses( xtraskillbonus );

            xtraskillbonus = (int)((int)from.Skills.ArmsLore.Base / 20);
            if (xtraskillbonus < 0)
                xtraskillbonus = 0;

            DistributeBonuses(xtraskillbonus);
			
			int xtradurability = (int)(((int)from.Skills.Blacksmith.Base - 100) * 1.5) + 5;
			if (xtradurability < 0)
				xtradurability = 0;
			this.MaxHitPoints += xtradurability;
			this.HitPoints += xtradurability;
