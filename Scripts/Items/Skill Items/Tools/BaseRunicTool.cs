		...
		public static void ApplyAttributesTo( BaseJewel jewelry, bool isRunicTool, int luckChance, int attributeCount, int min, int max )
		{
			...
			for ( int i = 0; i < attributeCount; ++i )
			{
				...
				switch ( random )
				{
					...
					case 15: ApplyAttribute( primary,	min, max, AosAttribute.Luck,					1, 100 ); break;
					case 16: ApplyAttribute( primary,	min, max, AosAttribute.SpellDamage,				1, 12 ); break;
					//case 18: ApplyAttribute( primary,	min, max, AosAttribute.NightSight,				1, 1 ); break;
					case 17: ApplySkillBonus( skills,	min, max, 0,									1, 15 ); break;
					case 18: ApplySkillBonus( skills,	min, max, 1,									1, 15 ); break;
					case 19: ApplySkillBonus( skills,	min, max, 2,									1, 15 ); break;
					case 20: ApplySkillBonus( skills,	min, max, 3,									1, 15 ); break;
					case 21: ApplySkillBonus( skills,	min, max, 4,									1, 15 ); break;
				}
			}
		}
	}
