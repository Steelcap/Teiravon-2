...
namespace Server.Items
{
	...
	public class InteriorDecorator : Item
	{
		...
		public static bool InHouse( Mobile from )
		{
			...
			return ( house != null && house.IsCoOwner( from )||staticHouse != null && staticHouse.IsCoOwner( from ) );
		}
		...
		private class InternalTarget : Target
		{
			...
			private static void Up( Item item, Mobile from )
			{
				...
				{
					StaticHouseRegion temp = (StaticHouseRegion)from.Region;

					if ( item.Location.Z < temp.m_House.HouseMaxHeight - 1 )
			...
			private static void Down( Item item, Mobile from )
			{
				...
				if ( from.Region is StaticHouseRegion )
				{
					...
					if ( item.Location.Z > temp.m_House.HouseMinHeight )
						item.Location = new Point3D( item.Location, item.Z -1 );
					else
