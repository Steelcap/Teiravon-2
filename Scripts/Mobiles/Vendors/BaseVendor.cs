...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
        ...
        private const int MaxCustomer = 4;

        private ArrayList m_Customers = new ArrayList();
        ...
        public ArrayList ExposeBuyInfo { get { return m_ArmorBuyInfo; } }
		...
		public virtual void VendorBuy( Mobile from )
		{
			...
			for ( int i = 0; i < playerItems.Count; ++i )
			{
				...
				foreach( IShopSellInfo ssi in sellInfo )
				{
					...
					{
						price = (int)(ssi.GetBuyPriceFor( item ) *( 1.0 - from.Skills.Begging.Value/500.0));
		...
		{
			if ( !IsActiveBuyer || from.Skills.Begging.Value < 40 )
				return;
				...
				foreach ( IShopSellInfo ssi in info )
				{
					...
					foreach ( Item item in items )
					{
						...
						if ( item.IsStandardLoot() && item.Movable && ssi.IsSellable( item ) )
							table[item] = new SellItemState( item, (int)(ssi.GetSellPriceFor( item ) * (1.0 + (from.Skills.Begging.Value/100.0))), ssi.GetNameFor( item ) );
					}
				}
		...
		{
			if ( !IsActiveBuyer  && seller.Skills.Begging.Value < 40)
				return false;
			...
			foreach ( SellItemResponse resp in list )
			{
				...
				foreach( IShopSellInfo ssi in info )
				{
					if ( ssi.IsSellable( resp.Item ) )
					{
                        ...
                        TotalMoney += (double)(ssi.GetSellPriceFor(resp.Item) * (1.0 + (seller.Skills.Begging.Value / 100))) * amount * .1;
						break;
					}
				}
			}
            ...
            if ( TotalMoney > 0)
			{
                ...
                GiveSilver = (int)(10 * (TotalMoney - GiveGold) );

                if (GiveSilver > 0)
                    seller.AddToBackpack(new Silver(GiveSilver));
			...
			//no cliloc for this?
            if (GiveGold > 0 && GiveSilver > 0)
			    SayTo( seller, true, "Thank you! I bought {0} item{1}. Here is your {2}gp and {3}sp.", Sold, (Sold > 1 ? "s" : ""), GiveGold, GiveSilver );

            else if (GiveGold > 0)
                SayTo(seller, true, "Thank you! I bought {0} item{1}. Here is your {2}gp.", Sold, (Sold > 1 ? "s" : ""), GiveGold);

            else
			    SayTo( seller, true, "Thank you! I bought {0} item{1}. Here is your {2}sp.", Sold, (Sold > 1 ? "s" : ""), GiveSilver );
