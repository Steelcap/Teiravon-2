...
namespace Server.Mobiles
{
	...
	public class PlayerVendor : Mobile
	{
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			/*
			int prewipe = m_HoldGold;
			m_HoldGold = (int)(m_HoldGold / 20 );
			Console.WriteLine( "Vendor gold removed: {0}", prewipe - m_HoldGold );
			*/
		...
		private void IsGem( Item dropped, out bool gem, out int value )
		{
			...
			for( int i=0; i < m_Gem.Length; i++ )
			{
				if ( dropped.GetType() == m_Gem[i] )
				{
					gem = true;
					value = (int)(dropped.Amount * (10 + i*5));
					return;
				}
			}
		...
		public override bool OnDragDrop( Mobile from, Item item )
		{
			...
			IsGem( item, out gem, out value );

			if (gem)
			{
				SayTo( from, "That's worth " + value + " gold of my services." );
				this.HoldGold += value;
				item.Delete();
				return true;
			}
		...
		public static void TryToBuy( Item item, Mobile from )
		{
			...
			else if ( vi.Created + TimeSpan.FromMinutes( 30.0 ) > DateTime.Now )
			{
				from.SendMessage( "You cannot buy this item right now.  Please wait thirty minutes and try again." );
			}
			else
			{
