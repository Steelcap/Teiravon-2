...
namespace Server.Mobiles
{
	public abstract class BaseHealer : BaseVendor
	{
		...
		public virtual void OfferResurrection( Mobile m )
		{
            ...
            {
                m.Resurrect();
                Say("Thou hast bonded thyself to this world, then leave not long thy master's side.");
                m.PlaySound(0x214);
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			if ( !m.Frozen && DateTime.Now >= m_NextResurrect && InRange( m, 4 ) && !InRange( oldLocation, 4 ) && InLOS( m ) )
			{
				...
				else if ( this.HealsYoungPlayers && m.Hits < m.HitsMax && m is PlayerMobile)
				{
