...
namespace Server.Mobiles
{
	...
	public class PlayerMobile : Mobile
	{
		...
		public PlayerMobile Looking;
		public PlayerMobile LookingAt;
		...
		{
            get { return (this.Skills.Alchemy.Base > 69.9); }
		...
		public bool Masonry
		{
            get { return (this.Skills.Carpentry.Base > 59.9); }
		...
		{
			get{ return (this.Skills.Mining.Base > 29.9); }
		...
		{
            get { return (this.Skills.Mining.Base > 59.9); }
		...
		{
            if (this is TeiravonMobile)
            {
                TeiravonMobile tav = this as TeiravonMobile;
                if ((tav.GetActivePotions(Server.Items.PotionEffect.Invisibility)))
                    return;
            }
		...
		*/
		...
		public override void SetLocation( Point3D loc, bool isTeleport )
		{
			...
			{
				SendMessage( "Error in Set Location, post exactly what you were doing to cause this message to appear on the bug forum." );
			}
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
            ...
            if (!from.Hidden && from is PlayerMobile && from != this && from.InLOS(this))
			{
                Manacles chains = null;
                if (from.FindItemOnLayer(Layer.TwoHanded) is Manacles)
                    chains = from.FindItemOnLayer(Layer.TwoHanded) as Manacles;
				PlayerMobile pm = (PlayerMobile)from;
				this.Looking = pm;
				pm.LookingAt = this;
                if (chains != null && chains.CheckCaught())
                {
                    this.SendMessage("{0} catches you escaping from your chains!", from.Name);
                    from.SendMessage("You catch {0} escaping from their chains and thwart their effort.", this.Name);
                }
                else
				this.SendMessage("{0} looks at you", from.Name);
				from.SendMessage("You turn your gaze at {0}", this.Name);
		...
		protected override bool OnMove( Direction d )
		{
			...
			if( Hidden )
			{
                if (!Mounted && Skills.Stealth.Value >= 25.0)
                {
                    ...
                    bool runOK = (tav.DruidFormGroup == 7 && tav.Shapeshifted) || (tav.HasFeat(TeiravonMobile.Feats.AdvancedStealth));

                    if (running && !runOK)
                    {
                        RevealingAction();
                    }
                    else if (running && runOK)
                    {
                    ...
                    if (Utility.RandomBool())
                    {
                        ...
                        if (checkers.Count > 0)
                        {
                            for(int i = 0; i < checkers.Count; i++)
                             {
                                TeiravonMobile c = checkers[i] as TeiravonMobile;
                                
                                if (SkillHandlers.Awareness.CheckSurroundings(c, this))
                                {
                                    c.Send(new LocationEffect((IPoint3D)this, 0x37C4, 1, 15, 0, 2));
                                    c.Send(new PlaySound(Utility.RandomList(0x127,0x128), (IPoint3D)c));
                                }
                            }
                        }
                    }
		...
		public bool CheckYoungProtection( Mobile from )
		{
			...
			if ( DateTime.Now - m_LastYoungMessage > TimeSpan.FromMinutes( 1.0 ) )
			{
				...
				SendMessage( "A monster looks at you menacingly but does not attack You would be under attack now if not for you being new to the realm of Teiravon." ); // A monster looks at you menacingly but does not attack.  You would be under attack now if not for your status as a new citizen of Britannia.
			}
		...
		public Point3D GetYoungDeathDestination()
		{
			...
			if ( this is TeiravonMobile )
			{
				TeiravonMobile m_Player = ( TeiravonMobile )this;

				Point3D loc;

				if ( m_Player.IsHuman() )
					loc = new Point3D( 2301, 988, 0 );

				else if ( m_Player.IsElf() )
					loc = new Point3D( 514, 2676, 0 );

				else if ( m_Player.IsOrc() || m_Player.IsGoblin() )
					loc = new Point3D( 2945, 2333, 2 );

				else if ( m_Player.IsDrow() )
					loc = new Point3D( 5224, 168, 5 );

				else if ( m_Player.IsDwarf() || m_Player.IsGnome())
					loc = new Point3D( 5210, 2962, 0 );
