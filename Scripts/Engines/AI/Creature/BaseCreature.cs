...
namespace Server.Mobiles
{
    ...
    public enum FightMode
    {
        ...
        Guard           // Attack things which attack players.
    }
    ...
    public class BaseCreature : Mobile
    {
        ...
        public virtual TimeSpan BondingAbandonDelay { get { return TimeSpan.FromDays(3.0); } }
        ...
        public virtual double BreathDamageScalar { get { return (Core.AOS ? 0.05 : 0.05); } }
        ...
        public virtual bool IsEnemy(Mobile m)
        {
            ...
            {
                // GuardAI
                if (this.FightMode == FightMode.Guard && m.Combatant is TeiravonMobile && !m.Combatant.Criminal)
                    return true;
                ...
                if (m is TeiravonMobile)
                {
                    ...
                    if (Spells.Third.BlessSpell.adalia.Contains(m_Player) && !Aggressors.Contains(m_Player))
                        return false;

                    //Disable NPC orcs from attacking orcs
                    if ((m_Player.IsOrc() || m_Player.IsGoblin()) && this.IsOrcNPC())
                        return false;

                    //Disable NPC spiders from attacking drow
                   // if (m_Player.IsDrow() && this.IsSpiderNPC())
                    //    return false;

                    //Disable NPC plants from attacking elves
                    if (m_Player.IsElf() && this.IsPlantNPC())
                        return false;

                    //Undead Bond feat functions
                    if (m_Player.IsNecromancer() && m_Player.HasFeat(TeiravonMobile.Feats.UndeadBond))
                    {
                        if (NecroFriendly(this, m_Player.PlayerLevel))
                            return false;
                    }

                    //Racial guard functions
                    if (this is OrogWarrior)
                    {
                    ...
                    else if (this is HumanGuard || this is HumanPaladin || this is DwarvenWarrior || this is ElvenFighter)
                    {
                        if (m_Player.IsOrc() || m_Player.IsDrow())
                            return true;
                        else
                    ...
                    else if (this is ElvenBladeMaster)
                    {
                        if (m_Player.IsElf())
                            return false;
                        else
                    ...
                    else if (this is DwarvenChampion)
                    {
                        if (m_Player.IsDwarf())
                            return false;
                        else
                    ...
                    else if (this is DrowFighter)
                    {
                        if (m_Player.IsDrow() || m_Player.IsOrc())
                            return false;
                        else
                            return true;
                    }
                    else if (this is DrowWeaponMaster)
                    {
                        if (m_Player.IsDrow())
                            return false;
                        else
                ...
                else if (m is BaseCreature)
                {
                    ...
                    //Disables NPC from attacking their own Mirror Images
                    if (m_Creature.SummonMaster == this)
                        return false;

                    //Disable NPC orcs from attacking PC orcs pets
                    if (this.IsOrcNPC() && m_Creature.ControlMaster is TeiravonMobile && m_Creature.Controled)
                    {
                        TeiravonMobile m_ControlMaster = (TeiravonMobile)m_Creature.ControlMaster;

                        if (m_ControlMaster.IsOrc() || m_ControlMaster.IsGoblin())
                            return false;
                    }

                    //Disable NPC spiders from attacking PC drow pets
                    if (this.IsSpiderNPC() && m_Creature.ControlMaster is TeiravonMobile && m_Creature.Controled)
                    {
                        TeiravonMobile m_ControlMaster = (TeiravonMobile)m_Creature.ControlMaster;

                        if (m_ControlMaster.IsDrow())
                            return false;
                    }

                    //Disable NPC plants from attacking PC elf pets
                    if (this.IsPlantNPC() && m_Creature.ControlMaster is TeiravonMobile && m_Creature.Controled)
                    {
                        TeiravonMobile m_ControlMaster = (TeiravonMobile)m_Creature.ControlMaster;

                        if (m_ControlMaster.IsElf())
                            return false;
                    }

                    //Disable NPC undead from attacking PC necromancer pets
                    if (m_Creature.Controled && m_Creature.ControlMaster is TeiravonMobile)
                    {
                        if (m_Creature.IsUndeadNPC() || m_Creature.IsDemonNPC())
                        {
                            TeiravonMobile m_ControlMaster = (TeiravonMobile)m_Creature.ControlMaster;

                            if (m_ControlMaster.IsNecromancer() && m_ControlMaster.HasFeat(TeiravonMobile.Feats.UndeadBond))
                            {
                                if (NecroFriendly(this, m_ControlMaster.PlayerLevel))
                                    return false;
                            }
                        }
                    }
            ...
            string data = "";
            XmlFriendly a = (XmlFriendly)XmlAttach.FindAttachment(this, typeof(XmlFriendly));
            if (a != null)
            {
                data = a.Data;
                a = (XmlFriendly)XmlAttach.FindAttachment(m, typeof(XmlFriendly));
        ...
        public bool NecroFriendly(BaseCreature creature, int level)
        {
            bool necrofriendly = false;
            int creaturelevel = 20;

            Type[] m_NecroFriendly = new Type[]
			{
				typeof( Skeleton ),
				typeof( RestlessSoul ),
				typeof( Zombie ),
            ...
            for (int i = 0; i < m_NecroFriendly.Length; i++)
            {
                if (creature.GetType() == m_NecroFriendly[i])
                {
                    necrofriendly = true;
                    creaturelevel = i + 1;
                    break;
            ...
            if (necrofriendly && (level > creaturelevel))
                return true;
        ...
        public override void Damage(int amount, Mobile from)
        {
            ...
            if (from is TeiravonMobile)
            {
                ...
                if (m_Player.HasFeat(TeiravonMobile.Feats.RacialEnemy) || m_Player.HasFeat(TeiravonMobile.Feats.ExtraRacialEnemy))
                {
                    bool racialenemy = false;

                    if (IsAnimalNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Animals || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Animals))
                        racialenemy = true;

                    else if (IsSnakeNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Serpents || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Serpents))
                        racialenemy = true;

                    else if (IsSpiderNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Spiders || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Spiders))
                        racialenemy = true;

                    else if (IsDragonNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Dragons || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Dragons))
                        racialenemy = true;

                    else if (IsTrollNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Giants || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Giants))
                        racialenemy = true;

                    else if (IsElementalNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Elementals || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Elementals))
                        racialenemy = true;

                    else if (IsUndeadNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Undead || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Undead))
                        racialenemy = true;

                    else if (IsDemonNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Daemons || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Daemons))
                        racialenemy = true;

                    else if (IsOrcNPC() && (m_Player.RacialEnemy == TeiravonMobile.RacialEnemies.Orcs || m_Player.ExtraRacialEnemy == TeiravonMobile.RacialEnemies.Orcs))
                        racialenemy = true;

                    if (racialenemy)
                    {
                        int extradamage = (int)((double)amount * ((20.0 + (double)(m_Player.PlayerLevel * 3) + (double)Utility.Random(10)) / 100.0));
                        amount += extradamage;

                        if (Utility.Random(6) == 0)
                            m_Player.SendMessage("You do " + extradamage + " extra damage against this creature.");
                    }
                }
            }
        ...
        {
            int val = 2 + ((TeiravonMobile)from).ExpertSkinning;
            int feathers = Feathers * val;
            int wool = Wool * val;
            int meat = Meat * val;
            int hides = Hides * val;
            int scales = Scales * val;
            ...
            else
            {
                ...
                if (hides != 0)
                {
                    HideType type = HideType;

                    if (scales != 0)
                        type = HideType.Scaled;

                    if (type == HideType.Regular)
                        corpse.DropItem(new Hides(hides));
                    else if (type == HideType.Spined)
                        corpse.DropItem(new SpinedHides(hides));
                    else if (type == HideType.Horned)
                        corpse.DropItem(new HornedHides(hides));
                    else if (type == HideType.Barbed)
                        corpse.DropItem(new BarbedHides(hides));
                    else if (type == HideType.Tufted)
                        corpse.DropItem(new TuftedHides(hides));
                    else if (type == HideType.Scaled)
                        corpse.DropItem(new ScaledHides(hides));
        ...
        private static double[] m_StandardActiveSpeeds = new double[]
			{
				0.05, 0.075, 0.08, 0.1, 0.15, 0.175, 0.25, 0.2, 0.3, 0.4
			};
        ...
        public virtual bool CheckFoodPreference(Item f)
        {
            ...
            if (f is CureHerb || f is HealHerb)
                return true;
        ...
        {
            if (!IsDeadPet && Controled && (ControlMaster == from || IsPetFriend(from)) && (dropped is Food || dropped is Gold || dropped is CureHerb || dropped is HealHerb || dropped is CookableFood || dropped is Head || dropped is LeftArm || dropped is LeftLeg || dropped is Torso || dropped is RightArm || dropped is RightLeg))
            {
                Item f = dropped;
                ...
                if (CheckFoodPreference(f))
                {
                    ...
                    if (f is HealHerb)
                    {
                        ((HealHerb)f).Drink(from);
                        return true;
                    }

                    if (f is CureHerb)
                    {
                        CureHerb c = f as CureHerb;
                        c.Drink(from);
                        return true;
                    }
                    ...
                    if (amount > 0)
                    {
                        ...
                        if (IsBondable && !IsBonded)
                        {
                            ...
                            if (master != null)
                            {
                                if (m_dMinTameSkill <= 29.1 || master.Skills[SkillName.AnimalTaming].Value >= m_dMinTameSkill || this is BaseMount)
                                {
        ...
        //STARTMOD: Teiravon creature type system

        public bool IsAnimalNPC()
        {

            Type[] m_Animal = new Type[]
			{
				typeof( Bird ),
				typeof( Chicken ),
				typeof( Eagle ),
        ...
        public bool IsUndeadNPC()
        {

            Type[] m_Undead = new Type[]
			{
				typeof( AncientLich ),
				typeof( Bogle ),
				typeof( BoneKnight ),
            ...
            for (int i = 0; i < m_Undead.Length; i++)
            {
                if (this.GetType() == m_Undead[i])
                    return true;
            }
        ...
        public bool IsOrcNPC()
        {
            ...
            for (int i = 0; i < m_Orc.Length; i++)
            {
                if (this.GetType() == m_Orc[i])
                    return true;
            }
        ...
        public bool IsSpiderNPC()
        {

            Type[] m_Spider = new Type[]
			{
				typeof( DreadSpider ),
				typeof( FrostSpider ),
            ...
            for (int i = 0; i < m_Spider.Length; i++)
            {
                if (this.GetType() == m_Spider[i])
                    return true;
            }
        ...
        public bool IsPlantNPC()
        {

            Type[] m_Plant = new Type[]
			{
				typeof( Reaper ),
				typeof( Bogling ),
				typeof( Corpser ),
        ...
        public bool IsDragonNPC()
        {

            Type[] m_Dragon = new Type[]
			{
				typeof( AncientWyrm ),
				typeof( Dragon ),
				typeof( Drake ),
            ...
            for (int i = 0; i < m_Dragon.Length; i++)
            {
                if (this.GetType() == m_Dragon[i])
                    return true;
            }
        ...
        public bool IsDemonNPC()
        {

            Type[] m_Demon = new Type[]
			{
				typeof( Balron ),
				typeof( Daemon ),
            ...
            for (int i = 0; i < m_Demon.Length; i++)
            {
                if (this.GetType() == m_Demon[i])
                    return true;
            }
        ...
        public bool IsTrollNPC()
        {

            Type[] m_Troll = new Type[]
			{
            ...
            for (int i = 0; i < m_Troll.Length; i++)
            {
                if (this.GetType() == m_Troll[i])
                    return true;
            }
        ...
        public bool IsSnakeNPC()
        {

            Type[] m_Snake = new Type[]
			{
				typeof( GiantSerpent ),
				typeof( IceSerpent ),
				typeof( IceSnake ),
        ...
        public bool IsElementalNPC()
        {

            Type[] m_Elemental = new Type[]
			{
            ...
            for (int i = 0; i < m_Elemental.Length; i++)
            {
                if (this.GetType() == m_Elemental[i])
                    return true;
            }
        ...
        //ENDMOD: Teiravon creature type system
        ...
        public void LuckLoot(Mobile critter, Container backpack, int luck, int level)
        {
            if (Utility.RandomMinMax(1, 150000) > luck)
                return;
            MetalGoldenChest luckchest = new MetalGoldenChest();
            luckchest.Locked = true;
            luckchest.LockLevel = luckchest.RequiredSkill = level * 5;
            if (level * 5 > 100)
                luckchest.LockLevel = luckchest.RequiredSkill = 100;
            luckchest.DropItem(new Gold(level * 20, level * 100));
            int itemchance = level * 5;
            if (itemchance > Utility.RandomMinMax(1, 1000))
            {
                Item item;
                item = Loot.RandomArmorOrShieldOrWeaponOrJewelry();
                int attributeCount = Utility.RandomMinMax(1, (int)(level / 4));
                int min = level * 1;
                int max = level * 5;
                if (max > 100)
                    max = 100;
        ...
        public override bool OnBeforeDeath()
        {
            ...
            //STARTMOD: TEIRAVON LOOT ADDITIONS
            ...
            if (!Summoned && !NoKillAwards && !IsBonded && IsPlantNPC())
            {
                for (int i = 0; i < 3; i++)
                {
                    int a = 1 + Utility.Random(4);
                    switch (Utility.Random(10))
                    {
                            
                        case 0: break;
                        case 1: PackItem(new FallenBranches(a));
                            break;
                        case 2: PackItem(new Lilypad(a));
                            break;
                        case 3: PackItem(new DriedVines(a));
                            break;
                        case 4: PackItem(new DriedHerbs(a));
                            break;
                        case 5: PackItem(new KnotGrass(a));
                            break;
                        case 6: PackItem(new Amaranth(a));
                            break;
                        case 7: PackItem(new Brambles(a));
                            break;
                        case 8: PackItem(new Rauwolfia(a));
                            break;
                        case 9: PackItem(new Mandrake(a));
                            break;
                        default: break;
                    }
                }
            }
            else if (!Summoned && !NoKillAwards && !IsBonded && IsAnimalNPC() && nodungeon)
            {
                int a = 1 + Utility.Random(4);
                switch (Utility.Random(10))
                {
            ...
            //ENDMOD: TEIRAVON LOOT ADDITIONS
        ...
        public static ArrayList GetLootingRights(ArrayList damageEntries, int hitsMax)
        {
            ...
            if (rights.Count > 0)
            {
                ...
                if (hitsMax >= 3000)
                    minDamage = topDamage / 24;
                else if (hitsMax >= 1000)
                    minDamage = topDamage / 12;
                else if (hitsMax >= 200)
                    minDamage = topDamage / 6;
                else
                    minDamage = topDamage / 3;
        ...
        private int CheckParty(TeiravonMobile m_Player)
        {
            ArrayList m_Damagers = this.DamageEntries;
            int m_PartySize = 0;
            int m_PartyAvgLevel = 0;

            Party m_Party = m_Player.Party as Party;

            if (m_Party == null)
                return m_Player.PlayerLevel;

            for (int i = m_Damagers.Count - 1; i >= 0; --i)
            {
                if (i >= m_Damagers.Count)
                    continue;

                DamageEntry de = (DamageEntry)m_Damagers[i];

                if (m_Party.Contains(de.Damager))
                {
                    TeiravonMobile m_PartyMember = (TeiravonMobile)de.Damager;

                    m_PartySize += 1;
                    m_PartyAvgLevel += m_PartyMember.PlayerLevel;
                }
            }
            ...
            m_PartyAvgLevel /= m_PartySize;

            return m_PartyAvgLevel;
        }

        public override void OnDeath(Container c)
        {
            int exp = 1;
            m_Level = TAVUtilities.CalculateLevel(this);

            exp = (m_Level * (m_Level + 1)) * 3;
            ...
            else
            {
                if (!Summoned && !m_NoKillAwards)
                {
                    ...
                    for (int i = 0; i < list.Count; ++i)
                    {
                        ...
                        TeiravonMobile m_Player = ds.m_Mobile as TeiravonMobile;
                        
                        double PopulationBonus = 1.0;

                        PopulationBonus = TAVUtilities.PopulationBonus(m_Player);
                        if (m_Player.PlayerLevel < 20)
                        {
                            if (PopulationBonus > 1.0)
                            {
                                int extra = (int)((exp * PopulationBonus) - exp);
                                if (Titles.AwardExp(m_Player, exp + extra))
                                    m_Player.SendMessage("You have gained {0} :({1} + {2}) experience", exp + extra, exp, extra);
                            }

                            else
                            {
                                if (Titles.AwardExp(m_Player, (int)(exp)))
                                    m_Player.SendMessage("You have gained {0} exp", exp);
                            }
                        }
                        if (m_Player.Young && m_Player.PlayerLevel >= 10)
                        {
                            Account acc = m_Player.Account as Account;

                            if (acc != null)
                            {
                                m_Player.SendMessage("You have reached a respectable level and have outgrown your status as a young player!");
        ...
        public virtual TimeSpan ReaquireDelay { get { return TimeSpan.FromSeconds(3.0); } }
        ...
        public virtual bool Rummage()
        {
            ...
            try
            {
                //Disable NPC orcs from looting PC orcs
                if ((toRummage != null) && (toRummage.Owner is TeiravonMobile) && this.IsOrcNPC())
                {
                    TeiravonMobile m_Corpse = (TeiravonMobile)toRummage.Owner;
                ...
                //Disable NPC spiders from looting PC drow
                if ((toRummage != null) && (toRummage.Owner is TeiravonMobile) && this.IsSpiderNPC())
                {
                    TeiravonMobile m_Corpse = (TeiravonMobile)toRummage.Owner;
            ...
            {
                Console.WriteLine("Error in basecreature rummage script.");
            }
            //
    ...
    public class LoyaltyTimer : Timer
    {
        ...
        {
            m_NextHourlyCheck = DateTime.Now + TimeSpan.FromHours(3.0);
        ...
        protected override void OnTick()
        {
            ...
            foreach (Mobile m in World.Mobiles.Values)
            {
                ...
                if (m is BaseCreature)
                {
                    ...
                    if (c.IsBonded && c.ControlMaster != null && ( c.ControlMaster.Skills[SkillName.AnimalTaming].Value < c.MinTameSkill && !(c is BaseMount)))
                    {
                        c.Say("*Cannot bond with a master of such low skill*");
                        c.IsBonded = false;
                    }

                    if (c.Map == Map.Internal && !c.Controled && c.ControlMaster == null && !c.IsStabled)
                    {
                        toRemove.Add(c);
                    }
