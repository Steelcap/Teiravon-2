	...
	public abstract class BaseAI
	{
		...
		public virtual void GetContextMenuEntries( Mobile from, ArrayList list )
		{
			if ( from.Alive && m_Mobile.Controled && from.InRange( m_Mobile, 14 ) )
			{
				if ( from == m_Mobile.ControlMaster )
				{
					...
					if ( !m_Mobile.Summoned && !m_Mobile.IsUndeadNPC() && !m_Mobile.IsDemonNPC() )
					{
		...
		public virtual void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( m_Mobile.Controled && m_Mobile.Commandable )
			{
                ...
                if (e.Mobile is TeiravonMobile && !isOwner)
                {
                    TeiravonMobile tav = e.Mobile as TeiravonMobile;
                    SlayerEntry Undead = SlayerGroup.GetEntryByName(SlayerName.Silver);
                    if (Spells.Third.BlessSpell.narindun.Contains(tav) && Undead.Slays(m_Mobile))
                        isOwner = true;
                }
						...
						switch ( keyword )
						{
							...
							case 0x16E: // *transfer
							{
								...
								if ( !m_Mobile.IsDeadPet && WasNamed( speech ) && m_Mobile.CheckControlChance( e.Mobile ) )
								{
									...
									else if ( m_Mobile.IsUndeadNPC() || m_Mobile.IsDemonNPC() )
										e.Mobile.SendMessage( "You cannot transfer that creature!" );
									else
...
/*Change Speed*/
		...
		public virtual bool DoOrderTransfert()
		{
			...
			if ( from != to && from != null && !from.Deleted && to != null && !to.Deleted && to.Player )
			{
				...
				*/
				if ( TransferItem.IsInCombat( m_Mobile ) )
				{
					from.SendMessage( "You may not transfer a pet that has recently been in combat." );
					to.SendMessage( "The pet may not be transfered to you because it has recently been in combat." );
				}
	...
	{
		if ( m_Mobile.ControlTarget == null || m_Mobile.ControlTarget.Deleted || !m_Mobile.ControlTarget.Alive || m_Mobile.ControlTarget.IsDeadBondedPet || !m_Mobile.InRange( m_Mobile.ControlTarget, m_Mobile.RangePerception * 2 ) || m_Mobile.ControlTarget == m_Mobile.ControlMaster )
		{
			m_Mobile.FocusMob = null;
			return false;
		}
		else
		{
			...
			if ( map != null )
			{
				...
				foreach ( Mobile m in eable )
				{
                    ...
                    if ((bCheckIt && (acqType == FightMode.Agressor || acqType == FightMode.Evil || acqType == FightMode.Guard)) || (!bCheckIt && m_Mobile.Summoned && !m_Mobile.Controled))
					{
                        if (acqType == FightMode.Agressor || acqType == FightMode.Evil || acqType == FightMode.Guard)
						{
							bCheckIt = false;
                            ...
                            if (acqType == FightMode.Guard && !bCheckIt)
                            {
                                if (m is BaseCreature && ((BaseCreature)m).Controled && ((BaseCreature)m).ControlMaster != null)
                                    bCheckIt = (((BaseCreature)m).ControlMaster.Combatant is TeiravonMobile && !m.Combatant.Criminal);
                                else
                                    bCheckIt = (m.Combatant is TeiravonMobile && !m.Combatant.Criminal);
                            }
						}
					}
