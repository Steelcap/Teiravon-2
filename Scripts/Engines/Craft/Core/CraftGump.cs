...
namespace Server.Engines.Craft
{
	public class CraftGump : Gump
	{
            ...
            : base((Int32)40, (Int32)40)
		{
			...
			else if ( m_CraftSystem == DefBowFletching.CraftSystem )
			{

				m_CraftSystem = new DefBowFletching();
				m_CraftSystem.CustomSystem( from );
			}
			else if (m_CraftSystem == DefBlacksmithy.CraftSystem)
			{
				m_CraftSystem = new DefBlacksmithy();
				m_CraftSystem.CustomSystem( from );
			}
			else if (m_CraftSystem == DefCarpentry.CraftSystem)
			{
				m_CraftSystem = new DefCarpentry();
				m_CraftSystem.CustomSystem( from );
			}
			else if (m_CraftSystem == DefTinkering.CraftSystem)
			{
				m_CraftSystem = new DefTinkering();
				m_CraftSystem.CustomSystem( from );
			}
			else if (m_CraftSystem == DefTailoring.CraftSystem)
			{
				m_CraftSystem = new DefTailoring();
				m_CraftSystem.CustomSystem( from );
			}
			else if (m_CraftSystem == DefCooking.CraftSystem)
			{
            ...
            // Polish option
            if (craftSystem.CanFinish)
            {
                AddButton(400, 342, 4005, 4007, GetButtonID(6, 9), GumpButtonType.Reply, 0);
                AddHtml(435, 345, 150, 18, "<BASEFONT COLOR=#FFFFFF>FINISH ITEM</BASEFONT>", false, false); // FINISH ITEM

                AddButton(400, 382, 4005, 4007, GetButtonID(6, 10), GumpButtonType.Reply, 0);
                AddHtml(435, 385, 150, 18, "<BASEFONT COLOR=#FFFFFF>POLISH ITEM</BASEFONT>", false, false); // POLISH ITEM
            }
            // ****************************************
			...
			if ( craftSystem.CraftSubRes.Init )
			{
				...
				if ( nameNumber > 0 )
					...
					AddHtml( 50, 365, 250, 18, "<basefont color=\"" + LabelColor + "\">" + nameString + "</basefont>", false, false );
			}
			// ****************************************
			...
			if ( items.Count > 0 )
			{
				for ( int i = 0; i < items.Count; ++i )
				{
					...
					if ( craftItem.NameNumber > 0 )
						...
						AddHtml( 255, 63 + (index * 20), 220, 18, "<basefont color=\"" + LabelColor + "\">" + craftItem.NameString + "</basefont>", false, false );
			...
			switch ( type )
			{
				...
				case 6: // Misc. buttons
				{
					switch ( index )
					{
                        ...
                        case 9: // Finish item
                        {
                                Finish.BeginTarget(m_From, system, m_Tool, false);

                            break;
                        }
                        case 10: // Polish item
                        {
                                Finish.BeginTarget(m_From, system, m_Tool, true);
