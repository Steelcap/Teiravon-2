	...
	public class CraftItem
	{
		...
		private static Type[][] m_TypesTable = new Type[][]
			{
				...
				new Type[]{ typeof( WhitePineLog ), typeof( WhitePineBoard ) },
				new Type[]{ typeof( AshwoodLog ), typeof( AshwoodBoard ) },
				new Type[]{ typeof( SilverBirchLog ), typeof( SilverBirchBoard ) },
				new Type[]{ typeof( YewLog ), typeof( YewBoard ) },
				new Type[]{ typeof( BlackOakLog ), typeof( BlackOakBoard ) },
		...
		private static Type[] m_ColoredItemTable = new Type[]
			{
				...
				typeof( BaseJewel ), typeof( DragonBardingDeed ), typeof( BaseRanged )
			};
		...
		private static Type[] m_ColoredResourceTable = new Type[]
			{
				...
				typeof( BaseLog ), typeof( BaseBoard )
			};
		...
		public double GetExceptionalChance( CraftSystem system, double chance, Mobile from )
		{
            if (from is TeiravonMobile)
            {
                ...
                if (tav.HasFeat(TeiravonMobile.Feats.Meticulous))
                {
                    f = 2;
                }
                    switch (system.ECA)
                    {
                        default:
                        case CraftECA.ChanceMinusSixty: return (chance - 0.6) * f;
                        case CraftECA.FiftyPercentChanceMinusTenPercent: return ((chance * 0.5) - 0.1) * f;
					...
					if ( badCraft <= 0 )
					{
						...
						if ( ConsumeRes( from, typeRes, craftSystem, ref resHue, ref maxAmount, ConsumeType.None, ref message ) )
						{
							...
							if ( ConsumeAttributes( from, ref message, false ) )
							{
								...
								int iMax = ( craftSystem.MaxCraftEffect - iMin ) + z;
		...
		public void CompleteCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool )
		{
			...
			if ( CheckSkills( from, typeRes, craftSystem, ref ignored, ref allRequiredSkills ) )
			{
				...
				if ( item != null )
				{
					...
					//STARMOD: Teiravon Crafting

					if ( from is TeiravonMobile )
					{
						TeiravonMobile m_Crafter = (TeiravonMobile)from;

						if ( m_Crafter.IsMerchant() || m_Crafter.IsAlchemist() || m_Crafter.IsBlacksmith() || m_Crafter.IsWoodworker() || m_Crafter.IsTailor() || m_Crafter.IsTinker() || m_Crafter.HasFeat(TeiravonMobile.Feats.AlchemyScience) || m_Crafter.IsCook() )
						{
							double chance = GetSuccessChance( from, typeRes, craftSystem, false, ref allRequiredSkills );
							int exp = (int)((((1.0 - chance) > 0 )? (1.0 - chance) : 0 ) * m_Crafter.PlayerLevel * 60);

							if (( exp > 0 ) && Titles.AwardExp(m_Crafter, exp))
								m_Crafter.SendMessage( "You have gained {0} exp", exp );
						}
                        //POLISH
                        m_Crafter.LastCrafted = item;
                        m_Crafter.LastCraftTime = DateTime.Now;
                        m_Crafter.NeedPolish = false;
					}

					bool furniture = false;

					MemberInfo info = item.GetType();
					...
					for (int i = 0; i < attributes.Length; i ++)
					{
						...
						if ( a != null && a is FurnitureAttribute )
							furniture = true;
					}

					if ( furniture && !(item is BaseContainer) )
					{
						...
						item.Hue = CraftResources.GetHue( CraftResources.GetFromType( resourceType ) );

						string ResourceProp = ((CraftResources.GetFromType( resourceType )).ToString()).ToLower();

						if (item.Name == null)
							item.Name = ( ResourceProp + " " + info.Name );
						else
							item.Name = ( ResourceProp + " " + item.Name );
					...
					{
						Shaft shaft = (Shaft)item;

						if ( maxAmount > 0)
							shaft.Amount = maxAmount;

						shaft.Amount *= (int)(from.Skills[SkillName.Fletching].Value / 20) + (int)(from.Skills[SkillName.Lumberjacking].Value / 20) + 1;

						from.AddToBackpack( shaft );

					}
                    else if (item is Longbow && craftSystem is DefBlacksmithy)
                    {
                        Longbow longbow = (Longbow)item;

                        longbow.Attributes.WeaponDamage = 100;
			...
			Type resourceType = typeRes;
			            ...
			            longbow.Resource = thisResource;
                        from.AddToBackpack(longbow);

                    }
					else if ( item is OrcSlingBullet )
					{
						OrcSlingBullet bullet = (OrcSlingBullet)item;

						if ( maxAmount > 0)
							bullet.Amount = maxAmount;

						bullet.Amount *= 2;

						from.AddToBackpack( bullet );

					}
					else if ( item is HumanAPBolt )
					{
						HumanAPBolt bolt = (HumanAPBolt)item;

						if ( maxAmount > 0)
							bolt.Amount = maxAmount;
					...
					else if ( item is DwarvenBolt )
					{
						DwarvenBolt bolt = (DwarvenBolt)item;
					...
					else if ( item is BaseWoodBoard )
					{
						...
						switch ( thisResource )
						{
                            case CraftResource.Pine: from.AddToBackpack(new PineBoard(maxAmount)); break;
                            case CraftResource.Redwood: from.AddToBackpack(new RedwoodBoard(maxAmount)); break;
                            case CraftResource.WhitePine: from.AddToBackpack(new WhitePineBoard(maxAmount)); break;
                            case CraftResource.SilverBirch: from.AddToBackpack(new SilverBirchBoard(maxAmount)); break;
                            case CraftResource.Yew: from.AddToBackpack(new YewBoard(maxAmount)); break;
                            case CraftResource.BlackOak: from.AddToBackpack(new BlackOakBoard(maxAmount)); break;
                            case CraftResource.Ashwood: from.AddToBackpack(new AshwoodBoard(maxAmount)); break;
                            default: from.AddToBackpack(new Board(maxAmount)); break;
						}

						item.Delete();
					}
					else if ( item is Pitcher )
					{
						...
						if ( CraftResources.GetFromType( resourceType ) != CraftResource.Iron )
							item.Hue = CraftResources.GetHue( CraftResources.GetFromType( resourceType ) );
						else
							item.Hue = 2401;
						...
						item.Name = ( ResourceProp + " Pitcher" );
