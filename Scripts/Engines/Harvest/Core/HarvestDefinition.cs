...
namespace Server.Engines.Harvest
{
	public class HarvestDefinition
	{
		...
		public HarvestBank GetBank( Map map, int x, int y )
		{
			...
			if ( bank == null )
//				banks[key] = bank = new HarvestBank( this, GetVeinAt() );
// randome ore veins add above line and subtract below line
...
//		public HarvestVein GetVeinAt()
		{
			if ( m_Veins.Length == 1 )
				return m_Veins[0];

			Random random = new Random( (x*17)+(y*11)+(map.MapID * 3) );
// random ore veins subtract above line, add below line, subtract second line below			
