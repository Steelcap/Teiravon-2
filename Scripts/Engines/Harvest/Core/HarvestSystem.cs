...
//						bank.Consume( def, item.Amount );
		...
		public HarvestResource GetRealResource( Type type, HarvestResource res, Mobile from )
		{
			try
			{
				HarvestResource[] allore = Mining.System.OreAndStone.Resources;

				for ( int i = 0; i < allore.Length; i++ )
				{
					HarvestResource ore = allore[i];
					Type[] types = ore.Types;
				...
				HarvestResource[] allwood = Lumberjacking.System.Definition.Resources;

				for ( int i = 0; i < allwood.Length; i++ )
				{
					HarvestResource wood = allwood[i];
					Type[] types = wood.Types;
			...
			{
				Console.WriteLine( "ERROR: Harvest system error." );
				from.SendMessage( "A mining error ocurred, please tell a scripter where you were mining." );
				return res;
			}
		}
