	...
	public class SmallBODAcceptGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 ) // Ok
			{
                ...
                if (cont.ConsumeTotal(typeof(Gold), 1000))
                {
                ...
                {
                    if (m_From.BankBox.ConsumeTotal(typeof(Gold), 1000))
                    {
                        if (m_From.PlaceInBackpack(m_Deed))
                        {
                            m_From.SendLocalizedMessage(1045152); // The bulk order deed has been placed in your backpack.
		...
		{
			if ( material >= BulkMaterialType.DullCopper && material <= BulkMaterialType.BlackOak )
