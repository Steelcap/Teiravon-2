...
namespace Server.Misc
{
	public class AutoSave : Timer
	{
		...
		public static void Save()
		{
            ...
            if (DateTime.Now.Hour == 1 && DateTime.Now.Minute > 29){
                try { Daily(); }
                catch {}
            }
			World.Save();
			...
			OleDbCommand m_SearchString = new OleDbCommand( "SELECT username, password, status FROM Applications WHERE status = 1", m_Database );
			...
			try
			{
				...
				ArrayList m_NewAccts = new ArrayList();

				while ( m_ApplicationList.Read() )
				{
					string username = m_ApplicationList["username"].ToString();
					string password = m_ApplicationList["password"].ToString();

					Account a = Accounts.AddAccount( username, password );

					m_NewAccts.Add( username );
				}

				m_ApplicationList.Close();

				foreach ( string user in m_NewAccts )
				{
					OleDbCommand m_ChangeStatus = new OleDbCommand( "UPDATE Applications SET status = 2 WHERE username = '" + user + "'", m_Database );
					m_ChangeStatus.ExecuteNonQuery();
				}
			...
			{
				Console.WriteLine( "CRITICAL ERROR: Database refresh error. " );
        ...
        private static void Daily()
        {
            ...
            string root = Path.Combine(Core.BaseDirectory, "Backups\\Daily");
            ...
            if (Directory.Exists(saves))
                Directory.Move(saves, FormatDirectory(root, "Daily ", GetTimeStamp()));
        }
