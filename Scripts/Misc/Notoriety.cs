...
namespace Server.Misc
{
	public class NotorietyHandlers
	{
		...
		public static bool Mobile_AllowHarmful( Mobile from, Mobile target )
		{
            ...
            if (Spells.Third.BlessSpell.adalia.Contains(target))
                return false;
            if (Spells.Third.BlessSpell.adalia.Contains(from))
            {
                from.SendMessage("You have broken your vow of pacifism, the goddess's blessing leaves you");
                Spells.Third.BlessSpell.adalia.Remove(from);
                from.FixedParticles(14276, 10, 20, 14276, EffectLayer.Waist);
                from.PlaySound(0x38F);
            }
