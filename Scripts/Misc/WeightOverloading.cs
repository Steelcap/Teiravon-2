	...
	public class WeightOverloading
	{
		...
		public static void FatigueOnDamage( Mobile m, int damage )
		{
			...
			switch ( m_DFA )
			{
				...
				{
					fatigue = (damage * ((100.0 / m.Hits) * ((double)m.Stam) / 200)) - 3.0;
		...
		public static int GetMaxWeight( Mobile m )
		{
            if (m is TeiravonMobile)
            {
                ...
                if (tav.IsCrafter() && tav.IsUndead() && tav.Shapeshifted)
                    return 40 + (int)(3.5 * m.Str) + (int)(20 * tav.PlayerLevel);
            }
		...
		public static void EventSink_Movement( MovementEventArgs e )
		{
            ...
            if (Server.Scripts.Commands.TeiravonCommands.dashing.Contains(from))
            {
                ...
                if (from.Stam < 15)
                {
                    Server.Scripts.Commands.TeiravonCommands.dashing.Remove(from);
                    from.Send(Server.Network.SpeedBoost.Disabled);
                }
            }
