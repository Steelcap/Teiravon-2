...
namespace Server.Engines.XmlSpawner2
{
    public class XmlCustomAttacks : XmlAttachment
    {
        ...
        public static new void Initialize()
        {
            ...
            AddSpecialAttack("Snap Kick", "A quick sharp kick to the chin, deals greater damage to unprotected heads.",
                SpecialAttacks.SnapKick, 0x5425, TimeSpan.FromSeconds(5),
                0, 20, 0, 0,
                20, 40, 0,
                new SkillName[] { SkillName.Wrestling, SkillName.Tactics },
                new int[] { 50, 50 },null,null
            );
            AddSpecialAttack("Dazing Strike", "A sudden jab leaving the target momentarily dazed.",
                SpecialAttacks.DazingStrike, 0x8CF, TimeSpan.FromSeconds(5),
                0, 20, 0, 0,
                20, 40, 0,
                new SkillName[] { SkillName.Wrestling, SkillName.Tactics },
                new int[] { 50, 50 }, null, null
            );
            AddSpecialAttack("Forceful Shout", "With a powerful yell you expell your latent ki around you.",
                SpecialAttacks.ForcefulShout, 0x5320, TimeSpan.FromSeconds(5),
                20, 10, 0, 0,
                10, 20, 0,
                new SkillName[] { SkillName.Wrestling, SkillName.Tactics },
                new int[] { 50, 50 }, null, null
            );
            AddSpecialAttack("Force Arrow", "An arrow imbued with potent force to knock the target away.",
                SpecialAttacks.ForceArrow, 0x5218, TimeSpan.FromSeconds(3.5),
                15, 0, 0, 0,
                20, 40, 0,
                new SkillName[] { SkillName.Archery, SkillName.Tactics },
                new int[] { 50, 50 }, null, null
            );
            AddSpecialAttack("Snare Arrow", "An arrow imbued with paralyzing forces to bind the target to the spot.",
                SpecialAttacks.SnareArrow, 0x5219, TimeSpan.FromSeconds(3.5),
                15, 0, 0, 0,
                20, 40, 0,
                new SkillName[] { SkillName.Archery, SkillName.Tactics },
                new int[] { 50, 50 }, null, null
            );
            AddSpecialAttack("Burst Shot", "An arrow imbued with explosive power to damage everything around the target.",
                SpecialAttacks.BurstShot, 0x8EA, TimeSpan.FromSeconds(3.5),
                15, 0, 0, 0,
                20, 40, 0,
                new SkillName[] { SkillName.Archery, SkillName.Tactics },
                new int[] { 50, 50 }, null, null
            );
            AddSpecialAttack("Snake Shot", "An arrow imbued with toxic forces to infect the target with virulent poison.",
                SpecialAttacks.SnakeShot, 0x521B, TimeSpan.FromSeconds(3.5),
                15, 0, 0, 0,
                20, 40, 0,
                new SkillName[] { SkillName.Archery, SkillName.Tactics },
                new int[] { 50, 50 }, null, null
            );
            AddSpecialAttack("Void Arrow", "An arrow imbued with negative forces to disperse arcane power.",
                SpecialAttacks.VoidArrow, 0x8F9, TimeSpan.FromSeconds(3.5),
                15, 0, 0, 0,
                20, 40, 0,
                new SkillName[] { SkillName.Archery, SkillName.Tactics },
                new int[] { 50, 50 }, null, null
            ...
            AddComboAttack("Squeezing Fist", ComboAttacks.SqueezingFist,
             ...
             AddComboAttack( "Burning Spirit", ComboAttacks.BurningSpirit,
                new SpecialAttacks []
                    {
                    SpecialAttacks.DazingStrike,
                    SpecialAttacks.ForcefulShout
                    }
            );
        ...
        private int ScaledDamage(int damage, int scale, Mobile attacker, Mobile defender)
        {

            int phys, fire, cold, pois, nrgy;
            ((BaseWeapon)attacker.Weapon).GetDamageTypes(attacker, out phys, out fire, out cold, out pois, out nrgy);

            int resPhys = AOS.Scale(defender.PhysicalResistance, scale);
            int resFire = AOS.Scale(defender.FireResistance, scale);
            int resCold = AOS.Scale(defender.ColdResistance, scale);
            int resPois = AOS.Scale(defender.PoisonResistance, scale);
            int resNrgy = AOS.Scale(defender.EnergyResistance, scale);
            int figNewtons;

            figNewtons = damage * phys * (100 - resPhys);
            figNewtons += damage * fire * (100 - resFire);
            figNewtons += damage * cold * (100 - resCold);
            figNewtons += damage * pois * (100 - resPois);
            figNewtons += damage * nrgy * (100 - resNrgy);
        ...
        public void DoSpecialAttack(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven, SpecialAttack special)
		{
            ...
            switch(special.AttackID)
            {
                ...
                {
                    defender.Damage(ScaledDamage(weapon.MaxDamage * 3,50, attacker, defender), attacker); break;
                }
                ...
                case SpecialAttacks.SnapKick:
                {
                    double damage = damageGiven;
                    if (defender.FindItemOnLayer(Layer.Helm) == null)
                        damage *= 1.5;
                    else
                        damage *= 0.5;

                    defender.Damage((int)damage, attacker);
                    attacker.PlaySound(0x535);
                    break;
                }
                case SpecialAttacks.DazingStrike:
                {
                    int minDamage, maxDamage;
                    attacker.Weapon.GetStatusDamage(defender, out minDamage, out maxDamage);
                    defender.Stam -= Utility.RandomMinMax(minDamage, maxDamage);
                    defender.Freeze(TimeSpan.FromSeconds(1));
                    attacker.PlaySound(0x534);
                    defender.PlaySound(0x204);
                    break;
                }
                case SpecialAttacks.ForcefulShout:
                {
                    int sound = attacker.Female ? 0x338 : 0x44a; 
                    attacker.PlaySound(sound);
                    ArrayList targets = new ArrayList();
                    Engines.PartySystem.Party p = Engines.PartySystem.Party.Get(attacker); 
                    foreach (Mobile m in attacker.GetMobilesInRange(1))
                    {
                        if (m.Blessed || m.AccessLevel > attacker.AccessLevel)
                            continue;
                        if (p != null && p.Contains(m))
                            continue;

                        if (m != attacker && attacker.InLOS(m))
                            targets.Add(m);
                    }
                    ...
                    if (targets.Count > 0)
                    {
                        ...
                        for (int i = 0; i < targets.Count; ++i)
                        {
                            ...
                            Gust push = new Gust((TeiravonMobile)attacker, m);
                            push.MoveToWorld(defender.Location, defender.Map);
                        }
                    }
                    break;
                }
                case SpecialAttacks.ForceArrow:
                {
                    int minDamage, maxDamage;
                    Gust push = new Gust((TeiravonMobile)attacker, defender);
                    push.MoveToWorld(defender.Location, defender.Map);
                    attacker.Weapon.GetStatusDamage(defender, out minDamage, out maxDamage);
                    int damage = Utility.RandomMinMax(minDamage, maxDamage) / 3;
                    defender.PlaySound(0x28E);
                    Effects.SendTargetEffect(defender, 0x37BE, 1, 4, 0x23, 3);
                    AOS.Damage(defender, attacker, damage, 0, 0, 0, 0, 100);
                    break;
                }
                case SpecialAttacks.SnareArrow:
                {
                    defender.Paralyze(TimeSpan.FromSeconds(5.0));

                    defender.FixedEffect(0x3967, 9, 32);
			        defender.PlaySound( 0x204 );
                    break;
                }
                case SpecialAttacks.BurstShot:
                {
                    int minDamage, maxDamage;
                    Effects.SendLocationEffect(defender.Location, defender.Map, 14013, 20);
                    Engines.PartySystem.Party p = Engines.PartySystem.Party.Get(attacker);
                    attacker.Weapon.GetStatusDamage(defender, out minDamage, out maxDamage);
                    int damage = (int)(Utility.RandomMinMax(minDamage, maxDamage) *.5);
                    try
                    {
                    ...
                    {
                        attacker.SendMessage("The shot misfires.");
                    }
                    break;
                }
                case SpecialAttacks.SnakeShot:
                {
                    int damage = damageGiven / 2;
                    defender.ApplyPoison(attacker, Poison.GetPoison(3));
                ...
                case SpecialAttacks.VoidArrow:
                {
                    defender.Mana -= damageGiven;

                    if (defender is BaseCreature)
                    {
                        bool dispellable = false;
                        BaseCreature m = defender as BaseCreature;
        ...
        public void DoComboAttack(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven, ComboAttack combo)
		{
		    ...
		    attacker.SendMessage("You unleash {0}!",combo.Name);
            ...
            switch(combo.AttackID)
            {
                ...
                case ComboAttacks.BurningSpirit:
                {
                    attacker.FixedEffect(0x3709, 9, 32);
                    defender.PlaySound(0x58E);
                    int Damage = (int)(attacker.Str * (double)(attacker.Stam / attacker.StamMax) * .35);
                    ArrayList targets = new ArrayList();
                    Engines.PartySystem.Party p = Engines.PartySystem.Party.Get(attacker); 
                    ...
                    foreach (Mobile m in attacker.GetMobilesInRange(2))
                    {
                        ...
                        if (attacker != m && attacker.InLOS(m))
                            targets.Add(m);
                    }
