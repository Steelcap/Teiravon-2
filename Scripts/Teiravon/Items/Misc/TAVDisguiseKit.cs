...
{
   public class DrowWhip : Item
   {
		[Constructable]
		public DrowWhip() : base( 0x166e )
		{
			Hue = 1432;
			Weight = 17.0;
		}

		public DrowWhip( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile src )
		{
			...
			{
				src.SendMessage("Who do you wish the snakes to strike?");
				src.Target = new WhipTarget( src );
				ArrayList list = new ArrayList();
				foreach ( Mobile mob in src.Map.GetMobilesInRange(src.Location, 5 ) )
				{
					list.Add(mob);
				}
				for(int i = 0; i < list.Count; i++)
				{
					...
					if (m.Hidden)
					{
						if (m.AccessLevel == AccessLevel.Player)
							m.Hidden = false;
						m.Say("The power of the whip has revealed {0} hidden in the shadows", m.Name);
					}
				}
			}
			else
		...
		public class WhipTarget : Target
		{
			private Mobile source;

			public WhipTarget( Mobile src ) : base( 5, false, TargetFlags.Harmful )
			{
				source = src;
			}
			...
			{
				source.Emote("You see the snakes recoil and hiss at their victim!");
			}
		}
	}
...
AddLabel( 120, 290, 0, "Name:" );
AddBackground( 155, 290, 150, 20, 0x2486 );
AddTextEntry( 160, 290, 140, 20, 0, 0, "" );
...
/* DYE button
			AddButton( 367, 353, 4005, 4007, 2, GumpButtonType.Reply, 0 );
			AddLabel( 400, 353, 0, "DYE" );
*/
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 0 )
			{
				...
				m_From.SendGump( new HairDyeGump( ) );

				if ( m_Used )
				{

					m_From.SendMessage( "Disguises wear off after 5 hours" ); // Disguises wear off after 2 hours.
				}
				else
...
*/
			...
			if ( index >= 0 && index < entries.Length )
			{
				...
				m_Timers[m_From] = Timer.DelayCall( TimeSpan.FromHours( 5.0 ), new TimerStateCallback( OnDisguiseExpire ), m_From );
