...
namespace Server.Items
{
	public class CustomNPCSpawner : Spawner
	{
        ...
        private int m_tokenhue = 0;
        private string m_tokenname = "quest token";
        private int m_tokenid = 10249;
        private string m_tokenkey = "key";
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public int TokenHue { get { return m_tokenhue; } set { m_tokenhue = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public string TokenName { get { return m_tokenname; } set { m_tokenname = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public int TokenID { get { return m_tokenid; } set { m_tokenid = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public string TokenKey { get { return m_tokenkey; } set { m_tokenkey = value; } }
        ...
        public override void Spawn(int index)
		{
			...
			{
                if (Creatures[i] is QuestTokenItem)
                {
                    QuestTokenItem token = (QuestTokenItem)Creatures[i];
                    token.Hue = TokenHue;
                    token.ItemID = TokenID;
				...
				if ( Creatures[ i ] is CustomNPC )
				{
					...
					if ( NPCName.ToLower() == "random" )
						...
						npc.Name = Female ? NameList.RandomName( "orcfemale" ) : NameList.RandomName( "orcmale" );
		...
		private void DoItems( CustomNPC npc )
		{
			foreach ( Item i in NPCItems.Items )
			{
                ...
                else if (i is QuestTokenItem) {
                    QuestTokenItem original = (QuestTokenItem)i;
                    QuestTokenItem temp = (QuestTokenItem)Activator.CreateInstance(i.GetType());
                ...
                else if (i is QuestScrollItem)
                {
                    QuestScrollItem original = (QuestScrollItem)i;
                    QuestScrollItem temp = (QuestScrollItem)Activator.CreateInstance(i.GetType());

                    temp.Name = original.Name;
                    temp.Hue = original.Hue;
                    temp.ItemID = original.ItemID;
                    temp.Key = original.Key;
                    temp.Addressee = original.Addressee;
                    temp.Message = original.Message;
                    temp.Sender = original.Sender;
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            //Version 5
            writer.Write(m_tokenhue);
            writer.Write(m_tokenname);
            writer.Write(m_tokenid);
            writer.Write(m_tokenkey);
		}
		...
		public override void Deserialize( GenericReader reader )
		{
            ...
            if (version >= 5)
            {
                m_tokenhue = reader.ReadInt();
                m_tokenname = reader.ReadString();
                m_tokenid = reader.ReadInt();
                m_tokenkey = reader.ReadString();
            }
            else
            {
                m_tokenhue = 0;
                m_tokenname = "quest token";
                m_tokenid = 10249;
                m_tokenkey = "key";
            }
	...
	public class CustomNPC : BaseCreature
	{
        ...
        private Container m_Container = null;
		private GuardControl m_Controller;
        ...
        private void DoItems()
        {
            ...
            foreach (Item i in NPCItems.Items)
            {
                ...
                else if (i is BaseClothing)
                {
                    ...
                    temp.Attributes = original.Attributes;
                    temp.ClothingAttributes = original.ClothingAttributes;
                    temp.SkillBonuses = original.SkillBonuses;
    ...
    public class NPCGrabBag : BaseContainer, IDyable
    {
        ...
        private int m_numberdropped;

        [CommandProperty(AccessLevel.GameMaster)]
        public int DropChance { get { return m_dropchance; } set { m_dropchance = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public int NumberDropped { get { return m_numberdropped; } set { m_numberdropped = value; } }
        ...
        public NPCGrabBag(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write((int)m_dropchance);
            writer.Write((int)m_numberdropped);
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            int version = reader.ReadInt();
            m_dropchance = reader.ReadInt();
            m_numberdropped = reader.ReadInt();
        }
    }
